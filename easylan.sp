...
public Plugin:myinfo =
{
	...
	author = "mrmatthew2k",
	description = "make lan easier for admins",
	version = "1.9",
...
public Action:TechnicalStop(int client, any args)
{
	...
	if (enabled && stop_enabled)
	{
    	//nothing
	}
	else
    {
       PrintToChat(client, "Easy Lan or the !stop function is disabled.");
       return Plugin_Handled;
    }
    // end of enabled check
	if (!gamestate_live)
...
public Action:TeamStay(int client, any args)
{
	...
	// check if my plugin is enabled
	new enabled = GetConVarBool(g_PluginEnabledConVar);
	if (enabled && gamestate_live_knifewinner > 1)
	{
    	//nothing
	}
	else
    {
       PrintToChat(client, "Easy Lan is disabled or there is not active knife round.");
       return Plugin_Handled;
    }
	int pteam;
	if (playersjoin.GetValue(authid[client], pteam) && pteam == gamestate_live_knifewinner)
	{
...
public Action:TeamSwitch(int client, any args)
{
    ...
    {
       PrintToChat(client, "Easy Lan is disabled or there is no active knife round.");
       return Plugin_Handled;
    }
	int pteam;
	if (playersjoin.GetValue(authid[client], pteam) && pteam == gamestate_live_knifewinner)
...
public Action:TeamReady(int client, any args)
{
	// check if my plugin is enabled
	new enabled = GetConVarBool(g_PluginEnabledConVar);
	if (enabled && gamestate_warmup)
	{
    	//nothing
	}
	else
    {
       PrintToChat(client, "Easy Lan is disabled or warmup mode is not active.");
       return Plugin_Handled;
    }
	int pteam;
	playersjoin.GetValue(authid[client], pteam);
	if (pteam == CS_TEAM_CT && gamestate_ct_ready == false)
    {
    	gamestate_ct_ready = true;
    	PrintToChatAll("%s are now ready", gamestate_ct_team);
    
    }
	if (pteam == CS_TEAM_T && gamestate_t_ready == false)
    {
...
public Action:TeamNotReady(int client, any args)
{
	...
	int pteam;
	playersjoin.GetValue(authid[client], pteam)
	if (pteam == CS_TEAM_CT && gamestate_ct_ready == true)
    {
    	gamestate_ct_ready = false;
    	PrintToChatAll("%s are not ready.", gamestate_ct_team);
    
    }
	if (pteam == CS_TEAM_T && gamestate_t_ready == true)
    {
